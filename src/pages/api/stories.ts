...
export default function handler(
  ...
  const usersData: UserData[] = Array.from({ length: Number(users) }, (_, index) => ({
    userId: index + 1,
    userProfileImage: `https://picsum.photos/300/200?random=${index + 1}`,
    stories: Array.from({ length: Math.floor(Math.random() * 5) + 1 }, (_, storyIndex) => ({ // Random number of stories per user (1 to 5)
      id: storyIndex + 1,
